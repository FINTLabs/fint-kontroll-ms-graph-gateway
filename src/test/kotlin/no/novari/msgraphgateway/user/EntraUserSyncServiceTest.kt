...
class EntraUserSyncServiceTest {
    ...
    fun processPageDeduplicatesUsersWithinSamePage() =
        runTest {
            ...
            assertEquals(1, published)
            verify(exactly = 1) { userRepository.batchUpsert(match { it.size == 1 && it.first().objectId == duplicateId }) }
            coVerify(exactly = 1) { producer.publish(any()) }
        }
    ...
    fun processPageRoutesExternalUsersToExternalRepositoryAndProducer() =
        runTest {
            ...
            assertEquals(2, published)
            verify(exactly = 1) { userRepository.batchUpsert(match { it.map { row -> row.objectId } == listOf(normalId) }) }
            verify(exactly = 1) {
